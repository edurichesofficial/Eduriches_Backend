...
exports.createUser = async (first_name, last_name, country, state, age, gender, phone, email, password, referred_by, contact_id) => {
  ...
  const checkExisting = await pool.query(
    `SELECT id FROM users WHERE email = $1 OR phone = $2 OR contact_id = $3`,
  ...
  const checkGuideCode = await pool.query(
    `SELECT id FROM users WHERE guide_code = $1`,
    [guide_code]
  );
  ...
  const result = await pool.query(
    `INSERT INTO users (
      first_name, last_name, country, state, age, gender,
      phone, email, password, referred_by, guide_code, contact_id
